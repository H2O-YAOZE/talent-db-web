...
def list_candidates(
    ...
    if keyword:
        where.append("(c.name LIKE ? OR c.email LIKE ? OR c.institution LIKE ? OR c.research_field LIKE ? OR c.github_username LIKE ? OR c.batch_name LIKE ?)")
        k = f"%{keyword}%"
        params.extend([k, k, k, k, k, k])
    if company:
...
def list_uploaders(username: str = __import__('fastapi').Depends(get_user)):
    conn = get_db()
    rows = conn.execute("SELECT uploaded_by, COUNT(*) as cnt FROM candidates WHERE source='resume' AND uploaded_by IS NOT NULL AND uploaded_by != '' GROUP BY uploaded_by ORDER BY cnt DESC").fetchall()
    conn.close()
    return {"data": [{"uploader": r["uploaded_by"], "count": r["cnt"]} for r in rows]}
